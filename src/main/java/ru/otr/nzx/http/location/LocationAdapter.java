...
public class LocationAdapter extends HttpFiltersAdapter {
	...
	public LocationAdapter(HttpRequest originalRequest, ChannelHandlerContext ctx, Map<String, LocationConfig> locations, Logger log) {
		...
		logLine.append(" ");
		logLine.append("REQ : from ");
		logLine.append(ctx.channel().remoteAddress().toString());
		logLine.append(" ");
		...
		logLine.append(" LEN=");
		logLine.append(originalRequest.headers().get(HttpHeaders.Names.CONTENT_LENGTH));
		logLine.append(" ");
		logLine.append(originalRequest.getDecoderResult().toString());
		log.info(logLine.toString());

		log.debug(requestID + " REQ : headers=" + originalRequest.headers().entries());
	...
	public HttpResponse clientToProxyRequest(HttpObject httpObject) {
		...
		if (httpObject instanceof HttpRequest) {
			...
			if (location != null && location instanceof ProxyPassLocationConfig) {
				ProxyPassLocationConfig loc = (ProxyPassLocationConfig) location;
				log.info(requestID + " PASS: proxy " + passURI);
				if (loc.proxy_set_headers.size() > 0) {
					...
					log.debug(requestID + " PASS: set headers=" + loc.proxy_set_headers);
				}
			...
			if (location == null) {
				...
				log.info(requestID + " RESP: proxy " + response.getProtocolVersion() + " [" + response.getStatus() + "]");
			}
		}
		if (dump_body_flag && httpObject instanceof FullHttpRequest) {

			FullHttpRequest request = (FullHttpRequest) httpObject;
			log.info(requestID + " DUMP: request LEN=" + request.headers().get(HttpHeaders.Names.CONTENT_LENGTH));
			...
			try (FileOutputStream dumpFOS = new FileOutputStream(dump)) {
				...
				log.info(requestID + " DUMP: request file=" + dump.toPath());
			} catch (IOException e) {
				log.error(requestID + " DUMP: request ERROR", e);
			}
		}
	...
	public HttpObject serverToProxyResponse(HttpObject httpObject) {
		if (httpObject instanceof HttpResponse) {
			...
			StringBuilder logLine = new StringBuilder();
			logLine.append(requestID);
			logLine.append(" RESP: server ");
			logLine.append(response.getProtocolVersion().toString());
			logLine.append(" [");
			logLine.append(response.getStatus());
			logLine.append("] LEN=");
			logLine.append(response.headers().get(HttpHeaders.Names.CONTENT_LENGTH));
			logLine.append(" ");
		...
		if (dump_body_flag && httpObject instanceof FullHttpResponse) {
			...
			log.info(requestID + " DUMP: response LEN=" + response.headers().get(HttpHeaders.Names.CONTENT_LENGTH));
			...
			try (FileOutputStream dumpFOS = new FileOutputStream(dump)) {
				...
				log.info(requestID + " DUMP: response file=" + dump.toPath());
			} catch (IOException e) {
				log.error(requestID + " DUMP: response ERROR=" + e.getClass().getName(), e);
			}
		}
		return httpObject;
	...
	public void serverToProxyResponseTimedOut() {
		...
		log.warn(requestID + " FAIL: server " + passURI + " ERROR=TimedOut");
	}

	@Override
	public void proxyToServerConnectionFailed() {
		super.proxyToServerConnectionFailed();
		log.warn(requestID + " FAIL: server " + passURI + " ERROR=ConnectionFailed");
	}
