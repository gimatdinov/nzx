    ...
    public HttpFilters filterRequest(HttpRequest request, ChannelHandlerContext ctx) {
        ...
        try {
            ...
            if (HttpMethod.CONNECT.equals(request.getMethod())) {
                return new FailureLocation(request, ctx, requestDateTime, requestID, requestURI, null, postProcessor, tracer.getSubtracer("#BadRequest"), 400);
            }
            ...
            if (location != null && location.enable) {
                ...
                return new FailureLocation(request, ctx, requestDateTime, requestID, requestURI, null, postProcessor, tracer.getSubtracer(location.path), 500);
            } else {
                return new FailureLocation(request, ctx, requestDateTime, requestID, requestURI, null, postProcessor,
                        tracer.getSubtracer("#LocationUnresolved"), 404);
            }
