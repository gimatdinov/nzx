...
public class PostProcessorConfig extends Config {
	...
	public PostProcessorConfig(JSONObject src, String route, final Map<String, Object> routes) throws URISyntaxException {
		super(route, routes);
		...
		routes.put(route + "/" + ACTIONS, actions);
		if (src.has(ACTIONS)) {
			JSONArray actArray = src.getJSONArray(ACTIONS);
			for (int i = 0; i < actArray.length(); i++) {
				actions.add(new ActionConfig(i, actArray.getJSONObject(i), route + "/" + ACTIONS, routes));
			}
		}
	}
