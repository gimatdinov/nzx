...
public class FTPServerConfig extends Config {
	...
	public FTPServerConfig(JSONObject src, String route, Map<String, Object> routes) throws URISyntaxException {
		...
		users = new ArrayList<>();
		routes.put(route + "/" + name + "/" + USERS, users);

		JSONArray userArray = src.optJSONArray(USERS);
		if (userArray != null) {
			for (int i = 0; i < userArray.length(); i++) {
				users.add(new FTPUserConfig(userArray.getJSONObject(i), route + "/" + name + "/" + USERS, routes));
			}
		}
	}
